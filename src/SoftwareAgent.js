...
import ReactFlow, { ReactFlowProvider, Controls, Background, applyNodeChanges, applyEdgeChanges, ConnectionLineType,Handle, Position } from 'reactflow';
...
const handleSubmit = async (e) => {
  ...
  if (questionInput.trim()) {
    try {
      const response = await axios.post('http://localhost:5088/self-healing-software', {
        method: 'GET',
      headers: {
  ...
  useEffect(() => {
    socket.on('update', data => {
      const newNode = {
        id: data.id,
        type: data.type,  // Use 'imageNode' if imageUrl is provided, otherwise default
        data: data.imageUrl ? { imageUrl: data.imageUrl } : { label: data.label }  // Pass the image URL or text label
      };

      const newEdge = data.parentId ? { id: `${data.parentId}-${data.id}`, source: data.parentId, target: data.id } : null;
...
function SoftwareFlowWithProvider(props) {
  return (
    <ReactFlowProvider>
      <App {...props} />
    </ReactFlowProvider>
...
export default SoftwareFlowWithProvider;
